...
PATH_MNIST = '~/code/datasets'
# 在调用自己的数据的时候，我们可以用torchvision.datasets.ImageFolder或者torch.utils.data.TensorDataset来加载
train_dataset = torchvision.datasets.MNIST(
    root=PATH_MNIST,  # 文件存放路径
    train=True,  # 提取训练集
    transform=torchvision.transforms.ToTensor(),  # 将图像转化为Tensor，在加载数据的时候，就可以对图像做预处理
    download=False)  # 当找不到文件的时候，自动下载
